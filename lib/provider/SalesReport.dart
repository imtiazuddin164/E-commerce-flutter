...
import '../service/SalesReportService.dart';
...
class SalesReportProvider with ChangeNotifier {
  ...
  List<SalesItemDetail> _detailedTodayList = [];

  bool get loading => _loading;
  double get todaySales => _todaySales;
  List<SalesItemDetail> get detailedTodayList => _detailedTodayList;
  ...
  Future<void> fetchDailySales() async {
    ...
    try {
      ...
      for (var order in orders) {
        final orderDate = DateTime.parse(order.date as String); // ধরলাম Order.date String
        if (orderDate.year == today.year &&
            orderDate.month == today.month &&
            orderDate.day == today.day) {
          _todaySales += order.totalAmount;
          for (var item in order.items) {
            _detailedTodayList.add(SalesItemDetail(
    ...
    try {
      ...
      for (var order in orders) {
        ...
        if (orderDate.year == today.year &&
          ...
          _todaySales += orderTotal; // Today total update
        }
      }
    } catch (e) {
      print("Error fetching daily sales: $e");
